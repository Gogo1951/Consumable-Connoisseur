...
if GetLocale() == "zhTW" then
    ...
    L["SCAN_HEALS"] = "治療([%d,]+)"
    L["SCAN_HEALTH"] = "生命力"
    L["SCAN_HEALTHSTONE"] = "治療石"
    L["SCAN_HPOT_STRICT"] = "恢復([%d,]+)至([%d,]+)點生命力"
    L["SCAN_HYBRID"] = "法力和生命力" 
    L["SCAN_LIFE"] = "生命力"
    L["SCAN_MANA"] = "法力"
    L["SCAN_MPOT_STRICT"] = "恢復([%d,]+)至([%d,]+)點法力"
    L["SCAN_PERCENT"] = "恢復(%d+)%%"
    L["SCAN_REQ_FA"] = "需要急救 %((%d+)%)"
    L["SCAN_REQ_LEVEL"] = "需要等級 (%d+)"
    L["SCAN_RESTORES"] = "恢復([%d,]+)"
    L["SCAN_SEATED"] = "必須保持坐姿"
    L["SCAN_USE"] = "使用："
    L["SCAN_WELL_FED"] = "進食充分"
end